...
<div class="wrap">
    <h1><?php _e('Alt Text Manager', 'wp-alttext-auditor'); ?></h1>
    <p class="description" style="margin-bottom: 0;"><?php _e('Manage alt-text for your media library images. Changes are saved automatically when you modify the alt-text field.', 'wp-alttext-auditor'); ?></p>
        ...
        <form method="get" action="">
            ...
            <div class="alttext-filter-row">
                <div class="alttext-filter-item">
                    <label for="alt-status-filter"><?php _e('Alt-Text Status:', 'wp-alttext-auditor'); ?></label>
                    <select name="alt_status" id="alt-status-filter">
                        <option value="all" <?php selected($alt_status, 'all'); ?>><?php _e('All Images', 'wp-alttext-auditor'); ?></option>
                        <option value="has_alt" <?php selected($alt_status, 'has_alt'); ?>><?php _e('Has Alt-Text', 'wp-alttext-auditor'); ?></option>
                        <option value="missing_alt" <?php selected($alt_status, 'missing_alt'); ?>><?php _e('Missing Alt-Text', 'wp-alttext-auditor'); ?></option>
                    </select>
                </div>

                <div class="alttext-filter-item alttext-search-item">
                    <label for="alttext-search"><?php _e('Search:', 'wp-alttext-auditor'); ?></label>
                    <input type="search" name="s" id="alttext-search" value="<?php echo esc_attr($search); ?>" placeholder="<?php _e('Search filename or alt-text...', 'wp-alttext-auditor'); ?>" />
                </div>

                <div class="alttext-filter-item">
                    <button type="submit" class="button"><?php _e('Apply Filters', 'wp-alttext-auditor'); ?></button>
                    <?php if ($alt_status !== 'all' || !empty($search)) : ?>
                        <a href="<?php echo esc_url(admin_url('admin.php?page=wp-alttext-auditor-manager')); ?>" class="button"><?php _e('Reset', 'wp-alttext-auditor'); ?></a>
                    <?php endif; ?>
                </div>
            </div>
        </form>
    ...
    <?php if ($media_query->have_posts()) : ?>
        <div class="alttext-stats">
            <p><?php printf(__('Showing %d of %d images', 'wp-alttext-auditor'), count($media_query->posts), $total_items); ?></p>
        </div>
        
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th scope="col" class="column-thumbnail"><?php _e('Thumbnail', 'wp-alttext-auditor'); ?></th>
                    <th scope="col" class="column-filename sortable <?php echo ($orderby === 'title') ? 'sorted' : ''; ?> <?php echo ($orderby === 'title') ? strtolower($order) : 'desc'; ?>">
                        <a href="<?php echo esc_url(add_query_arg(array('orderby' => 'title', 'order' => ($orderby === 'title' && $order === 'ASC') ? 'DESC' : 'ASC'))); ?>">
                            <span><?php _e('File Name', 'wp-alttext-auditor'); ?></span>
                            <span class="sorting-indicator"></span>
                        </a>
                    </th>
                    <th scope="col" class="column-alttext"><?php _e('Alt Text', 'wp-alttext-auditor'); ?></th>
                    <th scope="col" class="column-status sortable <?php echo ($orderby === 'alt_status') ? 'sorted' : ''; ?> <?php echo ($orderby === 'alt_status') ? strtolower($order) : 'desc'; ?>">
                        <a href="<?php echo esc_url(add_query_arg(array('orderby' => 'alt_status', 'order' => ($orderby === 'alt_status' && $order === 'ASC') ? 'DESC' : 'ASC'))); ?>">
                            <span><?php _e('Status', 'wp-alttext-auditor'); ?></span>
                            <span class="sorting-indicator"></span>
                        </a>
                    </th>
                </tr>
                ...
                <tr data-attachment-id="<?php echo esc_attr($attachment_id); ?>">
                    ...
                    <td class="column-filename">
                        ...
                        <div class="row-actions">
                            <span class="view">
                                <a href="<?php echo esc_url($attachment_url); ?>" target="_blank"><?php _e('View', 'wp-alttext-auditor'); ?></a> |
                            </span>
                            <span class="edit">
                                <a href="<?php echo esc_url(get_edit_post_link($attachment_id)); ?>"><?php _e('Edit', 'wp-alttext-auditor'); ?></a>
                            </span>
                        </div>
                    </td>
                    <td class="column-alttext">
                        <div class="alttext-input-wrapper">
                            <input 
                                ...
                                placeholder="<?php _e('Enter alt text...', 'wp-alttext-auditor'); ?>"
                                maxlength="255"
                            />
                    ...
                    <td class="column-status">
                        <span class="alttext-status <?php echo $has_alt ? 'has-alt' : 'no-alt'; ?>">
                            <?php if ($has_alt) : ?>
                                ...
                                <?php _e('Has Alt Text', 'wp-alttext-auditor'); ?>
                            <?php else : ?>
                                <span class="dashicons dashicons-warning"></span>
                                <?php _e('Missing Alt Text', 'wp-alttext-auditor'); ?>
                            <?php endif; ?>
                        </span>
                    </td>
                </tr>
    ...
    <?php else : ?>
        <div class="notice notice-info">
            <p><?php _e('No images found in your media library.', 'wp-alttext-auditor'); ?></p>
        </div>
    <?php endif; ?>
